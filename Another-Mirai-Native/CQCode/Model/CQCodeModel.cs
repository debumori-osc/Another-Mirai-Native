...
namespace Another_Mirai_Native.Adapter.CQCode.Model
{
	/// <summary>
	/// 表示 CQ码 的类
	/// </summary>
	public class CQCodeModel : IToSendString
	{
		#region --字段--
		...
		/// <summary>
		/// 获取一个值, 指示当前实例是否属于图片 <see cref="CQCodeModel"/>
		/// </summary>
		public bool IsImageCQCode { get { return EqualIsImageCQCode (this); } }

		/// <summary>
		/// 获取一个值, 指示当前实例是否属于语音 <see cref="CQCodeModel"/>
		/// </summary>
		...
		#region --构造函数--
		/// <summary>
		/// 使用 CQ码 字符串初始化 <see cref="CQCodeModel"/> 类的新实例
		/// </summary>
		/// <param name="str">CQ码字符串 或 包含CQ码的字符串</param>
		private CQCodeModel (string str)
		{
		...
		/// <summary>
		/// 初始化 <see cref="CQCodeModel"/> 类的新实例
		/// </summary>
		/// <param name="type">CQ码类型</param>
		/// <param name="keyValues">包含的键值对</param>
		public CQCodeModel (CQFunction type, params KeyValuePair<string, string>[] keyValues)
		{
			this._type = type;
		...
		#region --公开方法--
		/// <summary>
		/// 从字符串中解析出所有的 CQ码, 转换为 <see cref="CQCodeModel"/> 集合
		/// </summary>
		/// <param name="source">原始字符串</param>
		/// <returns>返回等效的 <see cref="List{CqCode}"/></returns>
		public static List<CQCodeModel> Parse (string source)
		{
			MatchCollection collection = _regices.Value[0].Matches (source);
			List<CQCodeModel> codes = new List<CQCodeModel> (collection.Count);
			foreach (Match item in collection)
			{
				codes.Add (new CQCodeModel (item.Groups[0].Value));
			}
			return codes;
		}
		/// <summary>
		/// 判断是否是图片 <see cref="CQCodeModel"/>
		/// </summary>
		/// <param name="code">要判断的 <see cref="CQCodeModel"/> 实例</param>
		/// <returns>如果是图片 <see cref="CQCodeModel"/> 返回 <see langword="true"/> 否则返回 <see langword="false"/></returns>
		public static bool EqualIsImageCQCode (CQCodeModel code)
		{
			return code.Function == CQFunction.Image;
		}
		/// <summary>
		/// 判断是否是语音 <see cref="CQCodeModel"/>
		/// </summary>
		/// <param name="code">要判断的 <see cref="CQCodeModel"/> 实例</param>
		/// <returns>如果是语音 <see cref="CQCodeModel"/> 返回 <see langword="true"/> 否则返回 <see langword="false"/></returns>
		public static bool EqualIsRecordCQCode (CQCodeModel code)
		{
		...
		{
			CQCodeModel code = obj as CQCodeModel;
			if (code != null)
			{
