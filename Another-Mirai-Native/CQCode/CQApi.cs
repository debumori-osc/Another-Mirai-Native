...
using Another_Mirai_Native.Adapter.CQCode.Interface;

namespace Another_Mirai_Native.Adapter.CQCode
{
	/// <summary>
	/// 表示 酷Q接口 的封装类
	/// </summary>
	public class CQApi
	{
		...
		/// <returns>返回 <see cref="CQCodeModel"/> 对象</returns>
		public static CQCodeModel CQCode_At (long qqId)
		{
			if (qqId < 0)
			{
				throw new ArgumentOutOfRangeException ("qqId");
			}
			return new CQCodeModel (
				CQFunction.At,
		...
		/// </summary>
		/// <returns>返回 <see cref="CQCodeModel"/> 对象</returns>
		public static CQCodeModel CQCode_AtAll ()
		{
			return new CQCodeModel (
				CQFunction.At,
		...
		/// <returns>返回 <see cref="CQCodeModel"/> 对象</returns>
		public static CQCodeModel CQCode_Emoji (int id)
		{
			return new CQCodeModel (
				CQFunction.Emoji,
		...
		/// <returns>返回 <see cref="CQCodeModel"/> 对象</returns>
		public static CQCodeModel CQCode_Face (CQFace face)
		{
			return new CQCodeModel (
				CQFunction.Face,
		...
		/// 获取酷Q "表情" 代码
		/// </summary>
		/// <param name="face">表情ID</param>
		/// <returns>返回 <see cref="CQCodeModel"/> 对象</returns>
		public static CQCodeModel CQCode_Face (int face)
		{
			return new CQCodeModel (
				CQFunction.Face,
		...
		/// </summary>
		/// <returns>返回 <see cref="CQCodeModel"/> 对象</returns>
		public static CQCodeModel CQCode_Shake ()
		{
			return new CQCodeModel (CQFunction.Shake);
		}
		/// <summary>
		/// 获取字符串副本的转义形式
		/// </summary>
		...
		/// <returns>返回 <see cref="CQCodeModel"/> 对象</returns>
		public static CQCodeModel CQCode_ShareLink (string url, string title, string content, string imageUrl = null)
		{
			...
			CQCodeModel code = new CQCodeModel (
				CQFunction.Share,
		...
		/// <returns>返回 <see cref="CQCodeModel"/> 对象</returns>
		public static CQCodeModel CQCode_ShareGPS (string site, string detail, double lat, double lon, int zoom = 15)
		{
		...
		/// <returns>返回 <see cref="CQCodeModel"/> 对象</returns>
		public static CQCodeModel CQCode_Anonymous (bool forced = false)
		{
			CQCodeModel code = new CQCodeModel (CQFunction.Anonymous);
			if (forced)
			{
		...
		/// <returns>返回 <see cref="CQCodeModel"/> 对象</returns>
		public static CQCodeModel CQCode_Music (long id, CQMusicType type = CQMusicType.Tencent, CQMusicStyle style = CQMusicStyle.Old)
		{
			return new CQCodeModel (
				CQFunction.Music,
				new KeyValuePair<string, string> ("id", Convert.ToString (id)),
		...
		/// <returns>返回 <see cref="CQCodeModel"/> 对象</returns>
		public static CQCodeModel CQCode_DIYMusic (string url, string musicUrl, string title = null, string content = null, string imageUrl = null)
		{
			if (string.IsNullOrEmpty (url))
			{
		...
		/// <returns>返回 <see cref="CQCodeModel"/> 对象</returns>
		public static CQCodeModel CQCode_Image (string path, bool flash = false)
		{
			...
			var c = new CQCodeModel (
				CQFunction.Image,
				new KeyValuePair<string, string> ("file", path));
            if (flash)
            {
				c.Items.Add("flash", "true");
            }
			return c;
		}
		/// <summary>
		...
		/// <exception cref="ArgumentException">参数: path 是空字符串或为 null</exception>
		/// <returns>返回 <see cref="CQCodeModel"/> 对象</returns>
		public static CQCodeModel CQCode_Record (string path)
		{
			if (string.IsNullOrEmpty (path))
			{
